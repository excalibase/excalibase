...
func (h *AuthHandler) CreateToken(w http.ResponseWriter, r *http.Request) {
	...
	raw, token, err := h.issueToken(r.Context(), user.ID, req.Name, "", lifetime)
	if err != nil {
...
// issueToken mints and persists a PAT bound to userID. The raw secret is
// returned exactly once and never stored.
func (h *AuthHandler) issueToken(ctx context.Context, userID, name, scopes string, lifetime auth.PATLifetime) (string, *domain.AccessToken, error) {
...
// name, scopes and lifetime (measured from now), then retires the old
...
func (h *AuthHandler) RotateToken(w http.ResponseWriter, r *http.Request) {
	...
	raw, fresh, err := h.issueToken(r.Context(), tok.UserID, tok.Name, tok.Scopes, auth.LifetimeOf(tok))
	if err != nil {
		httpError(w, safeError(err), http.StatusInternalServerError)
		return
	}
